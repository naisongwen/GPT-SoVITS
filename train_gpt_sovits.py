...
def open1Ba(batch_size,total_epoch,exp_name,text_low_lr_rate,if_save_latest,if_save_every_weights,save_every_epoch,gpu_numbers1Ba,pretrained_s2G,pretrained_s2D):
    ...
    if(p_train_SoVITS==None):
        ...
        "SoVITS训练开始：%s"%cmd,{"__type__":"update","visible":False},{"__type__":"update","visible":True}
        print(cmd)
...
def open1Bb(batch_size,total_epoch,exp_name,if_dpo,if_save_latest,if_save_every_weights,save_every_epoch,gpu_numbers,pretrained_s1):
    ...
    if(p_train_GPT==None):
        ...
        "GPT训练开始：%s"%cmd,{"__type__":"update","visible":False},{"__type__":"update","visible":True}
        print(cmd)
...
def open_slice(inp,opt_root,threshold,min_length,min_interval,hop_size,max_sil_kept,_max,alpha,n_parts):
    ...
    if os.path.isfile(inp):n_parts=1
        ...
        raise OSError("输入路径存在但既不是文件也不是文件夹")
...
if __name__ == "__main__":
    ...
    exp_root='/data/workdir/voice-clone'
    base_dir=f'{exp_root}/{voice_id}'
    ...
    SoVITS_weight_root=os.path.join(base_dir,SoVITS_weight_root)
    GPT_weight_root=os.path.join(base_dir,GPT_weight_root)
    os.makedirs(SoVITS_weight_root,exist_ok=True)
    os.makedirs(GPT_weight_root,exist_ok=True)
    SoVITS_names,GPT_names = get_weights_names()

    #tools/slice_audio.py "input/shenzhaojie.mp3" "output/slicer_opt" -34 4000 300 10 500 0.9 0.25 0 1
    #音频自动切分输入路径，可文件可文件夹
    slice_inp_path=audio_file_name
    #切分后的子音频的输出根目录
    slice_opt_root=f"{base_dir}/output/slicer_opt"
    #音量小于这个值视作静音的备选切割点
    threshold = -14
    min_length =4000
    #最短切割间隔
    ...
    alpha=0.25
    n_process=4
    open_slice(slice_inp_path,slice_opt_root,threshold,min_length,min_interval,hop_size,max_sil_kept,_max,alpha,n_process)
    asr_inp_dir =f"{base_dir}/output/slicer_opt"
    asr_opt_dir = f"{base_dir}/output/asr_opt"
    asr_model ='达摩 ASR (中文)'
    asr_size ='large'
    asr_lang ='zh'
    open_asr(asr_inp_dir, asr_opt_dir, asr_model, asr_size, asr_lang)
    ...
    gpu_numbers1Ba = "0"
    gpu_numbers1c ="0"
    open1abc(inp_text, inp_wav_dir,voice_id,gpu_numbers1a,gpu_numbers1Ba,gpu_numbers1c,bert_pretrained_dir,cnhubert_base_dir ,pretrained_s2G)
    ...
    #So VITS training
    open1Ba(batch_size,total_epoch,voice_id,text_low_lr_rate,if_save_latest,if_save_every_weights,save_every_epoch,gpu_numbers1Ba,pretrained_s2G,pretrained_s2D)

    batch_size1Bb = 8
    total_epoch1Bb = int(sys.argv[4])
    if_dpo =False
    ...
    #GPT training
    open1Bb(batch_size1Bb,total_epoch1Bb,voice_id,if_dpo,if_save_latest1Bb,if_save_every_weights1Bb,save_every_epoch1Bb,gpu_numbers1Bb,pretrained_s1)
    clean_proc=Popen('rm -rf logs* eval TEMP train.log events.out*',cwd=base_dir, shell=True)
    clean_proc.wait()
