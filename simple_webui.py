...
root_dir='/data/workdir/voice-clone'
...
voices=get_voices()
def get_weights_names(voice_id):
    SoVITS_names = [pretrained_sovits_name]
    
    SoVITS_weight_root="SoVITS_weights"
    GPT_weight_root="GPT_weights"
    
    SoVITS_weight_root=os.path.join(root_dir,voice_id,SoVITS_weight_root)
    for name in os.listdir(SoVITS_weight_root):
        if name.endswith(".pth"):SoVITS_names.append(name)
    GPT_names = [pretrained_gpt_name]

    GPT_weight_root=os.path.join(root_dir,voice_id,GPT_weight_root)
...
def change_choices(voice_id):
    SoVITS_names, GPT_names = get_weights_names(voice_id)
...
def start_train(voice_id,audio_input_path,sovits_total_epoch,gpt_total_epoch):
    ...
    if(p_train==None):
        import subprocess
        p_train = Popen(cmd, shell=True)  #,stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True)
        stdout, stderr  = p_train.communicate()
        info ="训练完成" if p_train.returncode==0 else "训练结束，但是中途出错了"
        p_train=None
        yield info,{"__type__":"update","visible":True},{"__type__":"update","visible":False}
    else:
        yield "已有正在进行的训练任务，需先终止才能开启下一次任务",{"__type__":"update","visible":False},{"__type__":"update","visible":True}

def close_train():
    global p_train
    if(p_train!=None):
        kill_process(p_train.pid)
        p_train=None
    return "已终止训练",{"__type__":"update","visible":True},{"__type__":"update","visible":False}
...
with gr.Blocks(title="GPT-SoVITS WebUI") as app:
    gr.Markdown(
        value=
            i18n("该软件以MIT协议开源, 作者不对软件具备任何控制力, 使用软件者、传播软件导出的声音者自负全责. <br>如不认可该条款, 则不能使用或引用软件包内任何代码和文件. 详见根目录<b>LICENSE</b>.")
    )

    with gr.Tabs():
            with gr.TabItem(i18n("微调训练")):
                with gr.Row():
                    input_audio_name = gr.Textbox(label=i18n("克隆声音名称"))
                    input_audio_path = gr.Textbox(label=i18n("训练音频路径"))
                    #batch_size = gr.Slider(minimum=1,maximum=40,step=1,label=i18n("每张显卡的batch_size"),value=default_batch_size,interactive=True)
                    sovits_total_epoch = gr.Slider(minimum=1,maximum=25,step=1,label=i18n("SoVITS 总训练轮数，不建议太高"),value=8,interactive=True)
                    gpt_total_epoch = gr.Slider(minimum=2,maximum=50,step=1,label=i18n("GPT 总训练轮数"),value=15,interactive=True)
                    #text_low_lr_rate = gr.Slider(minimum=0.2,maximum=0.6,step=0.05,label=i18n("文本模块学习率权重"),value=0.4,interactive=True)
                with gr.Row():
                    button_train = gr.Button(i18n("开启训练"), variant="primary",visible=True)
                    button_close = gr.Button(i18n("终止训练"), variant="primary",visible=False)
                    train_info=gr.Textbox(label=i18n("训练进程输出信息"))
            button_train.click(start_train,[input_audio_name,input_audio_path,sovits_total_epoch,gpt_total_epoch],[train_info,button_train,button_close])
            button_close.click(close_train,[],[train_info,button_train,button_close])

            with gr.TabItem(i18n("文本转语音")):
                with gr.Row():
                    voice_dropdown = gr.Dropdown(label=i18n("*声音列表"), choices=sorted(voices,key=custom_sort_key),interactive=True)
                    GPT_dropdown = gr.Dropdown(label=i18n("*GPT模型列表"),interactive=True)
                    SoVITS_dropdown = gr.Dropdown(label=i18n("*SoVITS模型列表"),interactive=True)
                    voice_dropdown.change(change_choices,[voice_dropdown],[SoVITS_dropdown, GPT_dropdown])
                with gr.Row():
                    inp_ref = gr.Audio(label=i18n("请上传3~10秒内参考音频，超过会报错！"), type="filepath")
                    prompt_text = gr.Textbox(label=i18n("参考音频的文本"), value="")
                with gr.Row():
                    text = gr.Textbox(label=i18n("需要合成的文本"), value="")
                    how_to_cut = gr.Radio(label=i18n("怎么切"),
                choices=[i18n("不断句"), i18n("按标点符号断句"), ],values=i18n("不断句"),
                interactive=True,
            )
                with gr.Row():
                    inference_button = gr.Button(i18n("语音生成"), variant="primary",visible=True)
                    output = gr.Audio(label=i18n("输出的语音"))
            inference_button.click(inference,[voice_dropdown,GPT_dropdown,SoVITS_dropdown,inp_ref, prompt_text, text, how_to_cut],
            [output],)
